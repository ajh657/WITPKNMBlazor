...
{
    public class PokemonApi(HttpClient http) : IPokemonApi
    {
        public async Task<PokemonData> GetPokemon(int number)
        {
            var request = await http.GetAsync($"https://pokeapi.co/api/v2/pokemon/{number}");
